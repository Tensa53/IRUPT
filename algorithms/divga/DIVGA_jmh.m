...
% parameters for jmh benchs (example programs)
...
% parameters for jmh benchs (real programs)
programs = {'avrojmhpre', 'avrojmhpost', 'hivejmhpre', 'hivejmhpost'};
program_config.avrojmhpre = struct('N', 333, 'M', 300, 'coverage_norm', 896, 'max_iterations', 334);
program_config.avrojmhpost = struct('N', 333, 'M', 300, 'coverage_norm', 892, 'max_iterations', 334);
program_config.hivejmhpre = struct('N', 414, 'M', 400, 'coverage_norm', 1459, 'max_iterations', 334);
program_config.hivejmhpost = struct('N', 414, 'M', 400, 'coverage_norm', 1464, 'max_iterations', 334);

%for each program
%costs is an array of size N
%coverage is an array of size N
function total_fitness = computeFitness(population, costs, coverage, norm_factor)
    [m, n] = size(population);
...
function f = fitnessFunction(x, costs, coverage, norm_factor)
    ...
    for rep = 1:10
        ...
        for i = 1:max_iterations
            ...
            for z = 1:size(population, 1)
                fitness_values(z) = computeFitness(population(z, :), costs, coverage, norm_factor);
            end
